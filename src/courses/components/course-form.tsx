...
import { courseService, type GenerateCourseData } from '../services/course-service';
...
export function CourseForm({
  ...
  const [lastAIData, setLastAIData] = useState<GenerateCourseData | null>(null);
  ...
  const handleGenerateCourse = async (data: GenerateCourseData) => {
    try {
      setIsGenerating(true);
      setLastAIData(data);
      setHasGenerated(true);

      const generatedCourse = await courseService.generateCourse(data);
    ...
    <>
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {isNewCourse && (
              <div className="flex gap-2">
                {hasGenerated && lastAIData && (
                  <button
                    type="button"
                    onClick={handleRegenerate}
                    className="inline-flex items-center px-3 py-1.5 text-sm font-medium text-gray-600 bg-gray-50 rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors cursor-pointer"
                  >
                    <svg
      ...
      >
        <GenerateCourseForm
          ...
          initialData={lastAIData || undefined}
        />
      </Modal>
    </>
  );
